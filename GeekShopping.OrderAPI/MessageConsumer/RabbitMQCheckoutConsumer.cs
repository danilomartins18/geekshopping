    ...
    public class RabbitMQCheckoutConsumer : BackgroundService
    {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += (channel, evt) =>
            {
                ...
                CheckoutHeaderMessage message = JsonSerializer.Deserialize<CheckoutHeaderMessage>(content);
                ProcessOrder(message).GetAwaiter().GetResult();
        ...
        private async Task ProcessOrder(CheckoutHeaderMessage message)
        {
            OrderHeader order = new()
            {
                ...
                Email = message.Email,
                ExpiryMonthYear = message.ExpiryMothYear,
                OrderTime = DateTime.Now,
                PurchaseAmount = message.PurchaseAmount,
                PaymentStatus = false,
                Phone = message.Phone,
                OrderDate = message.DateTime
            };

            foreach (var details in message.CartDetails)
            {
